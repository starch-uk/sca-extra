...
This project will provide additional PMD and Regex rules for testing Salesforce
Apex code using Salesforce Code Analyzer. Most rules will be implemented as
XPath 3.1 expressions that operate on the PMD Apex AST. Some rules will use the
Regex engine for pattern-based matching.
...
**IMPORTANT:** This project will use PMD CLI directly as an external tool. PMD
is NOT an npm package and must be installed separately.
...
4. **Note for Tests:**
    ...
    - Test helper should check for PMD availability and provide clear error
      messages
...
1. **Initialize pnpm project**
    ...
    - Add dev dependencies:
        ...
        - `sanitize-filename` (^1.6.3) - File path sanitization for scripts
...
4. **Create `.gitignore`**
    - Ignore `node_modules/`, test artifacts, AST dumps, benchmark results,
...
6. **Set up pre-commit hooks**
    ...
    - Configure `.lintstagedrc` or `package.json` lint-staged section to format
      XML files with Prettier
...
7. **Set up GitHub Actions CI/CD**
    ...
    - Run on pull requests: lint (Prettier check), ESLint, and unit tests with
      coverage
    - Fail PR if files are not properly formatted or tests fail
    - Upload coverage to Codecov (requires CODECOV_TOKEN secret)
    - **Note:** Benchmark PR comments and performance regression alerts will not
      be implemented in CI (benchmarking can be run manually)
...
8. **Create `README.md`**
    ...
    - **What Makes a Good Rule vs. What Prettier Handles** (important guidance
      section)
...
11. **Create GitHub issue templates**
    ...
    - `.github/ISSUE_TEMPLATE/feature_request.md` - Template for feature
      requests
...
2. **Create test helper functions** (`tests/helpers/pmd-helper.js`)
    - `runPMD(rulesetPath, apexFile)` - Execute PMD CLI and return results
        - Uses
          `pmd check --no-cache -d <file> -R <ruleset> -f xml -r <output-file>`
    ...
    - `assertViolation(violations, ruleName, lineNumber)` - Assert specific
      violation exists
...
4. **Generate combined test ruleset**
    - Create `scripts/generate-test-ruleset.js` to combine all rulesets into
      single XML
...
5. **Multi-line formatting rules** (SingleArgumentMustBeSingleLine,
...
2. **Add linting and formatting**
    - ESLint 9.x with flat config (`eslint.config.mjs`) for JavaScript test
      files
...
- **Command format:**
  `pmd check --no-cache -d <apex-file> -R <ruleset> -f xml -r <output-file>`
    ...
    - `-r <file>` - Output report to file (required to avoid progressbar
      conflicts)
...
- **Helper script:** Create `scripts/ast-dump.sh`
    ...
    - Usage: `pnpm run ast-dump <apex-file>` or
...
1. **XPath Only**: All rules MUST use XPath 3.1 expressions only. No custom Java
   classes or rule implementations beyond `XPathRule` are permitted.
...
4. **XML Element Order**: Rules must follow PMD Ruleset XML Schema element
   order:
    - `<description>` → `<priority>` → `<properties>` → `<exclude>` →
      `<example>`
...
6. **Benchmark Coverage**: All rules should be included in benchmark stress
   tests:
...
5. **Usage**
    ...
    - Baseline generation: `pnpm run benchmark -- --baseline` (creates
      baseline.json)
    - Compare mode: `pnpm run benchmark -- --compare` (doesn't fail on
      regressions)
    - Check regressions: `pnpm run check-regressions` (compares against
      baseline)
...
1. **P1** (Critical): InnerClassesCannotBeStatic,
   InnerClassesCannotHaveStaticMembers
...
- **Regex Rules:** 3 rules (NoConsecutiveBlankLines, ProhibitSuppressWarnings,
  NoLongLines) - defined in code-analyzer.yml
...
1. **Code Formatting Check**: Verifies all XML rulesets are formatted with
   Prettier
...
- [ ] Benchmark PR comments and performance regression alerts in CI will not be
      implemented (benchmarking can be run manually)
...
This project will be licensed under the **MIT License**. The `LICENSE.md` file
will contain the full license text.

...
async function benchmark() {
  ...
  for (const rule of rules) {
    ...
    for (let iteration = 0; iteration < results.iterations; iteration++) {
      ...
      const simulatedTime = 5 + iterationViolations.length * 0.1 + (totalFileSize / 1024) * 0.05;
  ...
  if (fs.existsSync(baselinePath)) {
    ...
    results.rules.forEach((rule) => {
      ...
      if (baselineRule) {
        ...
        const baselineTime = baselineRule.executionTime || baselineRule.executionTimeMean || 0;
  ...
  if (usePMD) {
    ...
    for (const fixture of fixtures) {
      try {
        ...
        console.warn(`Warning: Could not run PMD for ${rule.name} on ${fixture}: ${error.message}`);
...
function simulateViolations(rule, fixturePath) {
  ...
  if (ruleName === "ProhibitSuppressWarnings") {
    estimatedViolations = (content.match(/@SuppressWarnings/g) || []).length;
  } else if (ruleName === "NoMethodCallsInConditionals") {
    ...
    const abbreviations = ["ctx", "idx", "msg", "cfg", "val", "acc", "con", "opp", "param", "len"];
...
function printBenchmarkResults(results) {
  ...
  console.log("─".repeat(120));
  const slowest = [...results.rules].sort((a, b) => b.executionTime - a.executionTime).slice(0, 5);
